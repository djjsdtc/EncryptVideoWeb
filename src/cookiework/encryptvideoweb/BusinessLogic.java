...
public class BusinessLogic {
    ...
    public static ArrayList<SubscriptionInfo> getMyFinalize(String username) throws SQLException {
        ...
        Connection conn = Util.getConn();
        PreparedStatement query = conn.prepareStatement("select destUserID, MPrime, id, status from subscriptions where (status = ? or status = ?) AND userID = ?");
        //String dbQuery = "select destClientID, MPrime, id, tagName from subscriptions where status = 1 AND clientID ='" + ClientID +"'";
        query.setString(1, SUBSCRIPTION_APPROVE);
        query.setString(2, SUBSCRIPTION_REPLACE);
    ...
    public static String getPublicKey(String username) throws SQLException {
        ...
        ResultSet results = query.executeQuery();
        if (results.next()) {
            //publicKey = results.getString("publicKey");
            publicKey = "success";
        }
        conn.close();
    ...
    //获取关注我的人的列表
    public static ArrayList<SubscriptionInfo> getMyFollowers(String login) throws SQLException {
        ArrayList<SubscriptionInfo> followers = new ArrayList<>();

        Connection conn = Util.getConn();
        PreparedStatement query = conn.prepareStatement("select userID, id from subscriptions where status = ? AND destUserID = ?");
        query.setString(1, SUBSCRIPTION_FINALIZE);
        query.setString(2, login);
        //String dbQuery = "select tag, tagName, destClientID, id from subscriptions where status = 2 AND clientID = '" +login+ "';";
        //query.executeQuery(dbQuery);
    ...
    public static ArrayList<VideoInfo> getMyMessages(String destUser, String type) throws SQLException {
        ...
        } else if(type.equals("live_pending")) {
            query = conn.prepareStatement("select id, cipherTitle, cipherIntro, cipherAddr, status from storedvideos where userID=? and (status=? or status=?)");
            query.setString(2, "live");
    ...
    //public static void addNewPublisher(String publicKey, String username, String pass) throws SQLException {
    public static void addNewPublisher(String username, String pass) throws SQLException {
        ...
        Connection conn = Util.getConn();
        //PreparedStatement query = conn.prepareStatement("Insert Into publishers (publicKey, password, clientID) Values (?,?,?)");
        PreparedStatement query = conn.prepareStatement("Insert Into publishers (password, clientID) Values (?,?)");
        //query.setString(1, publicKey);
    ...
    protected static String changePrivateKeyInside(String mStr, String cStr){
        long beginTime = System.currentTimeMillis();

        final ECCurve curve = new SecP256K1Curve();
        ECPoint m = curve.decodePoint(UrlBase64.decode(mStr));
        BigInteger c = new BigInteger(cStr);
        ECPoint mPrime = m.multiply(c);
        String result = new String(UrlBase64.encode(mPrime.getEncoded(true)));
    ...
    public static String changePrivateKey(String id, String mStr) throws SQLException {
        Connection conn = Util.getConn();
        PreparedStatement stmt = conn.prepareStatement("select c from subscriptions where id=?");
        stmt.setInt(1, Integer.parseInt(id));
        ResultSet rs = stmt.executeQuery();
        rs.next();
        String cStr = rs.getString("c");
        String mPrimeStr = changePrivateKeyInside(mStr, cStr);
        updateSubscription(mPrimeStr, id);
        return mPrimeStr;
    }

    public static void recordTagReplace(String cStr, String[] sub_ids) throws SQLException{
        Connection conn = Util.getConn();
        for(String sub_id_str : sub_ids){
            int sub_id = Integer.parseInt(sub_id_str);
            PreparedStatement stmt = conn.prepareStatement("update subscriptions set status=?, c=? where id=?");
            stmt.setString(1, SUBSCRIPTION_REPLACE);
            stmt.setString(2, cStr);
    ...
    public static void removeSubscription(String[] sub_ids) throws SQLException{
        Connection conn = Util.getConn();
        for(String sub_id_str : sub_ids){
            int sub_id = Integer.parseInt(sub_id_str);
            PreparedStatement stmt = conn.prepareStatement("update subscriptions set status=? where id=?");
            stmt.setString(1, SUBSCRIPTION_REFUSE);
